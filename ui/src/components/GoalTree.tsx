...
function GoalNode({ goal, children, allGoals, depth, goalLink, onSelect, onDelete }: GoalNodeProps) {
  ...
  const inner = (
    <>
      ...
      {onDelete && (
        <button
          className="opacity-0 group-hover/goal:opacity-100 p-0.5 text-muted-foreground hover:text-destructive transition-opacity"
          title="Delete goal"
          onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
  ...
  const classes = cn(
    "group/goal flex items-center gap-2 px-3 py-1.5 text-sm transition-colors cursor-pointer hover:bg-accent/50",
  );
